...
var _ = Describe("Metal3Remediation manager", func() {
	...
	DescribeTable("Test Finalizers",
		func(tc testCaseRemediationManager) {
			remediationMgr, err := NewRemediationManager(nil, tc.Metal3Remediation, nil,
				logr.Discard(),
			)
			Expect(err).NotTo(HaveOccurred())
	...
	DescribeTable("Test GetUnhealthyHost",
		func(tc testCaseGetUnhealthyHost) {
			...
			remediationMgr, err := NewRemediationManager(fakeClient, nil, tc.Machine,
				logr.Discard(),
			)
			Expect(err).NotTo(HaveOccurred())
	...
	Describe("Test PowerOffAnnotation", func() {
		...
		m3machine := &machinev1beta1.Machine{
			ObjectMeta: metav1.ObjectMeta{
				Name:            "mym3machine",
				Namespace:       namespaceName,
				OwnerReferences: []metav1.OwnerReference{},
		...
		It("should set and remove the power off annotation as requested", func() {
			...
			remediationMgr, err := NewRemediationManager(fakeClient, remediation, m3machine,
				logr.Discard(),
			)
			Expect(err).NotTo(HaveOccurred())
	...
	Describe("Test NodeBackupAnnotation", func() {
		It("should set and remove the node backup annotation as requested", func() {
			...
			remediationMgr, err := NewRemediationManager(fakeClient, m3Remediation, nil,
				logr.Discard(),
			)
			Expect(err).NotTo(HaveOccurred())
	...
	Describe("Test Nodes", func() {
		...
		It("Should find, update and delete node", func() {
			fakeClient := fake.NewClientBuilder().WithScheme(setupScheme()).WithObjects(cluster, m3Remediation, machine, node).Build()
			remediationMgr, err := NewRemediationManager(fakeClient, m3Remediation, machine,
				logr.Discard(),
			)
			Expect(err).NotTo(HaveOccurred())

			By("Getting node")
			node, err := remediationMgr.GetNode(context.TODO())
			...
			Expect(remediationMgr.UpdateNode(context.TODO(), node)).To(Succeed(), "should update node without error")

			key := client.ObjectKey{Name: node.Name}
			newNode := &corev1.Node{}
			err = fakeClient.Get(context.TODO(), key, newNode)
			...
			By("Deleting node")
			Expect(remediationMgr.DeleteNode(context.TODO(), node)).To(Succeed(), "should delete node without error")
			err = fakeClient.Get(context.TODO(), key, newNode)
