...
func (bmr *BareMetalRemediator) Reboot(ctx context.Context, machineRemediation *mrv1.MachineRemediation) error {
	klog.V(4).Infof("MachineRemediation %q has state %q", machineRemediation.Name, machineRemediation.Status.State)
	...
	case mrv1.RemediationStateStarted:
		...
		if !bmh.Spec.Online && !rebootInProgress {
			klog.V(4).Infof("Skip the remediation, machine %q has power off state before the remediation action", machine.Name)
			bmr.recorder.Eventf(
				machine,
			...
			klog.V(4).Infof("Power off machine %q", machine.Name)
	...
	case mrv1.RemediationStatePowerOff:
		...
		if machineRemediation.Status.StartTime.Time.Add(rebootDefaultTimeout * time.Minute).Before(now) {
			klog.Errorf("Remediation of machine %q failed on timeout", machine.Name)
			bmr.recorder.Eventf(
				machine,
		...
		if bmh.Status.PoweredOn {
			klog.Warningf("machine %q still has power on state", machine.Name)
			return nil
		}
		...
		klog.V(4).Infof("Power on machine %q", machine.Name)
	...
	case mrv1.RemediationStatePowerOn:
		...
		if err != nil {
			...
			if errors.IsNotFound(err) {
				klog.Warningf("The machine %q node does not exist", machine.Name)
				return nil
			}
			return err
		}
